...
| 옵션 | 설명 |
|------|------|
| `oa ide` | code-server (브라우저 IDE) 시작 |
| `oa ide --stop` | code-server 종료 |
| `oa ide --status` | code-server 실행 상태 확인 |
| `oa opencode` | OpenCode 시작 |
| `oa opencode --stop` | OpenCode 종료 |
...
`bash install.sh`를 실행하면 아래 10단계가 순서대로 실행됩니다.

### [1/10] 환경 체크 — `scripts/check-env.sh`
...
### [2/10] 패키지 설치 — `scripts/install-deps.sh`
...
### [3/10] glibc 환경 — `scripts/install-glibc-env.sh`
...
### [4/10] 경로 설정 — `scripts/setup-paths.sh`
...
### [5/10] 환경변수 설정 — `scripts/setup-env.sh`
...
glibc 아키텍처에서는 `NODE_OPTIONS`, `CFLAGS`, `CXXFLAGS`, `GYP_DEFINES`, `CPATH`가 더 이상 필요하지 않습니다 — 이들은 이전 Bionic 아키텍처에서 필요했지만 표준 glibc 환경에서는 불필요합니다.

### [6/10] OpenClaw 설치 및 패치 — `npm install` + `patches/apply-patches.sh`
...
### [7/10] code-server 설치 — `scripts/install-code-server.sh`
...
설치 후 `oa ide`로 code-server를 시작할 수 있습니다.
...
설치 후 `oa opencode`로 OpenCode를 시작할 수 있습니다.

### [9/10] 설치 검증 — `tests/verify-install.sh`
...
`oa --update` (또는 하위 호환을 위한 `oaupdate`)를 실행하면 GitHub에서 `update-core.sh`를 다운로드하여 아래 9단계를 순서대로 실행합니다. 전체 설치와 달리 환경 체크, 경로 설정, 검증을 생략하고 — 패치, 환경변수, 패키지 갱신에만 집중합니다.
...
### [8/9] code-server 업데이트 (IDE)

`install-code-server.sh`를 `update` 모드로 실행하여 code-server를 설치하거나 업데이트합니다. 이미 설치되어 있고 최신 상태면 이 단계는 스킵됩니다. 비필수 — 실패 시 경고만 출력.

### [9/9] OpenCode + oh-my-opencode 설치

`install-opencode.sh`를 실행하여 OpenCode와 oh-my-opencode를 설치하거나 업데이트합니다. glibc 아키텍처가 필요하며, 마이그레이션에 실패한 Bionic 설치에서는 스킵됩니다. 비필수.

</details>

## 보너스: 폰에서 AI CLI 도구 사용

이 프로젝트가 설치하는 glibc 환경은 표준 Linux 런타임을 제공하여, 주요 AI CLI 도구를 설치하고 실행할 수 있습니다:
